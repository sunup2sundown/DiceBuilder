...
/**
 * Created by M on 5/1/2017.
 */

public class DownloadDiceDialog extends DialogFragment {
    private final String TAG = "DownloadDiceDialog";
    private EditText friendName;
    DownloadDiceDialog.DownloadDiceDialogListener mListener;

    /*The activity that Shares instance of dialog must implement
    * this interface in order to recieve event callbacks
     */
    public interface DownloadDiceDialogListener{
    ...
    public void onAttach(Activity activity){
        ...
        try{
            ...
            mListener = (DownloadDiceDialog.DownloadDiceDialogListener) activity;
        } catch(ClassCastException e){
            //Activity doesn't implement
            throw new ClassCastException(activity.toString() + " must implement DownloadDiceDialogListener");
        }
    }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState){
        ...
        builder.setView(inflater.inflate(R.layout.dialog_downloaddice, null))
                .setPositiveButton("Ok", new DialogInterface.OnClickListener(){
                    public void onClick(DialogInterface dialog, int id){
                        //Send the positive button event back to host activity
                        mListener.onDownloadDicePositiveClick(DownloadDiceDialog.this);
                    }
                })
                .setNegativeButton("Cancel", new DialogInterface.OnClickListener(){
                    public void onClick(DialogInterface dialog, int id){
                        //send the negative button event back to host activity
                        mListener.onDownloadDiceNegativeClick(DownloadDiceDialog.this);
                    }
                });
